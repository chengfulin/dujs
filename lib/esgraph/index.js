...
/// FIXME: FAILED to integrate with FlowNode
/**
...
function ControlFlowGraph(astNode) {
	...
	function eachType(type) {
		...
		if (type === FlowNode.ON_EVENT_CONNECTION_TYPE && next.length !== 0) {
	...
	function createNodes(astNode) {
		walker(astNode, { default: function (node, recurse) {
			...
			if (node.type === 'FunctionDeclaration' || node.type.indexOf('Expression') !== -1) {
				return;
			}
	...
	function createNode(astNode, parent) {
		if (!astNode.cfg) {
			Object.defineProperty(astNode, 'cfg', {
				value: factoryFlowNode.create(FlowNode.NORMAL_NODE_TYPE, astNode, parent), configurable: true
			});
		}
	}
