...
/**
 * Convert links of a node to JSON format
 * @returns {string}
 */
FlowNode.prototype.linksToJSON = function () {
	"use strict";
	var thisNode = this;
	var links = [];
	FlowNode.CONNECTION_TYPES.forEach(function (connectType) {
		if (FlowNode.MULTI_CONNECTION_TYPE.indexOf(connectType) === -1 && !!thisNode[connectType]) {
			links.push({"from": thisNode, "to": thisNode[connectType], "type": connectType});
		} else if (FlowNode.MULTI_CONNECTION_TYPE.indexOf(connectType) !== -1) {
			thisNode[connectType].forEach(function (node) {
				links.push({"from": thisNode, "to": node, "type": connectType});
			});
		}
	});
	return links;
