...
function findDUPairs(cfgWrapper) {
    ...
    cfgWrapper.getCFG()[2].forEach(function (node) {
        ...
        var nodeRD = cfgWrapper.getReachIns().get(node),
            ...
            nodeCUse = getUsedDefs(nodeRD, nodeUSE.cuse),
            nodePUse = getUsedDefs(nodeRD, nodeUSE.puse);
        /// Initialization
        ...
        nodeCUse.values().forEach(function (elem) {
            ...
            pairs.add(dupairFactory.create(elem.definition.getFromCFGNode(), node.cfgId));
            dupairs.set(elem.variable, pairs);
        });
        ...
        nodePUse.values().forEach(function (elem) {
            var pairs = dupairs.get(elem.variable);
            /// Assume each id of CFG nodes will be different
            pairs.add(dupairFactory.create(elem.definition.getFromCFGNode(), pairFactory.create(node.cfgId, node.true.cfgId)));
            pairs.add(dupairFactory.create(elem.definition.getFromCFGNode(), pairFactory.create(node.cfgId, node.false.cfgId)));
            dupairs.set(elem.variable, pairs);
        });
    });
    return dupairs;
