 ...
 */
var factoryModel = require('./modelfactory'),
 ...
 * @returns {Map} Intra-procedural models of pages
 */
ModelBuilder.prototype.buildIntraProceduralModels = function () {
    "use strict";
    var intraProceduralModelsOfPages = new Map();
	scopeCtrl.pageScopeTrees.forEach(function (scopeTree) {
		...
		intraProceduralModelsOfPages.set(scopeTree, models);
	});
    return intraProceduralModelsOfPages;
};
        ...
        if (!!cfgNode.astNode && cfgNode.type !== FlowNode.ENTRY_NODE_TYPE && cfgNode.type !== FlowNode.EXIT_NODE_TYPE) {
            walkes(cfgNode.astNode, {
                CallExpression: function (node) {
                    var callee = cfgNode.scope.getVariable(node.callee.name);
                    if (!!callee) {
 ...
 * @returns {Array} Inter-procedural modesl of pages
 */
ModelBuilder.prototype.buildInterProceduralModels = function () {
    "use strict";
    var interProceduralModelsOfPages = [];
	scopeCtrl.pageScopeTrees.forEach(function (scopeTree) {
		var models = [];
		...
		while (scopes.length > 0) {
			...
			if (relatedScopes.length > 1) {/// there is any inter-procedurals indeed
				...
				var analyzedCFG = factoryModel.create();
				if (!!analyzedCFG) {
		...
		interProceduralModelsOfPages.push(models);
	});
    return interProceduralModelsOfPages;
};
