...
function ModelBuilder() {
    ...
    this._testonly_ = {
		...
		_connectPageRelatedModelToLoopNode: connectPageRelatedModelToLoopNode
    };
    /* end-test-block */
}
...
/**
 * Connect the LOOP NODE and the related model of a page
 * @param {Model} modelForPage
 * @returns {Model}
 * @memberof ModelBuilder.prototype
 * @private
 */
function connectPageRelatedModelToLoopNode(modelForPage) {
	"use strict";
	...
	resultModel.graph = [modelForPage.graph[0], exitNode, nodes];
	resultModel.addRelatedScope(modelForPage.mainlyRelatedScope);
	modelForPage.relatedScopes.forEach(function (scope) {
