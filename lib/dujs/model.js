...
/**
 * Represent graph with JSON format
 * @returns {Object}
 */
Model.prototype.graphToJSON = function () {
	"use strict";
	var jsonObject = {
		"nodes": [].concat(internal(this)._graph[2]),
		"links": []
	};
	jsonObject.nodes.forEach(function (node) {
		jsonObject.links = jsonObject.links.concat(node.linksToJSON());
	});
	return jsonObject;
};

/**
 * Represent Def-Use pairs with JSON format
 * @returns {Object}
 */
Model.prototype.dupairsToJSON = function () {
	"use strict";
	var jsonArray = [];
	internal(this)._dupairs.forEach(function (pair, variable) {
		var setOfPairs = null;
		var hasNewSetOfPairs = jsonArray.every(function (obj) {
			if (obj.variable === variable) {
				setOfPairs = obj.dupairs;
				return false;
			}
			return true;
		});
		if (hasNewSetOfPairs) {
			jsonArray.push({"variable": variable, "dupairs": [pair]});
		} else {
			setOfPairs.push(pair);
		}
	});
	return jsonArray;
};
...
/**
 * Represent the model with JSON format
 * @returns {Object}
 */
Model.prototype.toJSON = function () {
	"use strict";
	return {"graph": this.graphToJSON(), "dupairs": this.dupairsToJSON()};
};
/* end-public-methods */
