...
function connectCallerCalleeCFGs(callerCFG, calleeCFG, callSiteNode) {
    ...
    if (callerCFG[2].indexOf(callSiteNode) !== -1 && FlowNode.isFlowNode(callSiteNode)) {
        ...
        callSiteNode.clearGENSet();
        callSiteNode.clearKILLSet();
 ...
 * @function
 */
function findInterProceduralRealatedScopesAndCFG(currentScope, scopeTree, analysisItemCtrl, connectToCFG) {
    "use strict";
    if (!connectToCFG) {
                ...
                CallExpression: function (node) {
                    ...
                    if (!!callee) {
                        cfgNode.reachIns.some(function (varDef) {
                            if (varDef.variable === callee && varDef.definition.type === Def.FUNCTION_TYPE) {
                                var scope = scopeTree.getScopeByDef(varDef.definition);
                                var scopeAnalysisItem = analysisItemCtrl.getIntraProceduralAnalysisItemByTopRelatedScope(scope);
                                if (!!scope) {
                                    if (!!scope.cfg[0].extraReachIns) {

                                        scope.cfg[0].extraReachIns = Set.union(scope.cfg[0].extraReachIns, cfgNode.reachIns);
                                    } else {

                                        scope.cfg[0].extraReachIns = new Set(cfgNode.reachIns);
                                    }
                                    DefUseAnalyzer.doAnalysis(scopeAnalysisItem);
                                    connectedCFG = [].concat(connectCallerCalleeCFGs(connectToCFG, scope.cfg, cfgNode));
                                    var recursiveResult = findInterProceduralRealatedScopesAndCFG(scope, scopeTree, analysisItemCtrl, connectedCFG);
 ...
 * @function
 */
AnalyzedCFGBuilder.prototype.buildInterProceduralAnalysisItems = function (analysisItemCtrl, scopeTree) {
    "use strict";
    ...
    if (ScopeTree.isScopeTree(scopeTree)) {
        ...
        while (scopes.length > 0) {
            var relatedScopes,
                connectedCFG;
            var foundReault = findInterProceduralRealatedScopesAndCFG(scopes[0], scopeTree, analysisItemCtrl);
