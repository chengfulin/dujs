...
function Var(name, scope, liveWith) {
    ...
    console.log(Scope.PROGRAM_SCOPE_TYPE);
 ...
 */
Var.validate = function (name, scope, liveWith, msg) {
    'use strict';
    if (!Var.isValidName(name)) {
        throw new Error(msg || 'Invalid name for a Var');
    }
    //if (!Scope.isScope(scope)) {
    //    throw new Error(msg || 'Invalid scope for a Var');
    //}
    if (!Var.isValidForLiveWith(liveWith)) {
        throw new Error(msg || 'Invalid liveWith Var for a Var');
    }
};
