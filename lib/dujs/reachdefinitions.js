...
function findReachDefinitions(cfgWrapper, entryRDs) {
    ...
    return worklist(cfgWrapper.getCFG(), function (input) {
        if (this.type || !this.astNode) {
            /// TODO: if we need to know where the definition actually comes from, for inter-procedural, ReachOut(exit) should contain definitions generated at return statement only.
            if (this.type === 'exit') {
                return new Set();
            }
            return input;
