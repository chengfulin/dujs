...
function ScopeWrapper(cfg, scope) {
    ...
    internal(this)._initialVars = new Map(); /// (name, Var)
    internal(this)._children = []; /// [ScopeWrappers]
...
ScopeWrapper.validate = function (cfg, scope, msg) {
    ...
    if (!CfgExt.isValidCFG(cfg)) {
        throw new Error(msg || 'Invalid CFG value for a ScopeWrapper');
    }
    try {
 ...
 * @function
 */
ScopeWrapper.prototype.setInitGlobalVars = function (initGlobals) {
    'use strict';
    if (initGlobals instanceof Array || initGlobals instanceof Set) {
        var thisScopeWrapper = this,
            allString = false,
            allVar = false;
            //entryRds = internal(this).reachIns.get(internal(this)._cfg[0]) || new Set();
        initGlobals.forEach(function (elem) {
            if (VarDef.isVarDef(elem)) {
                ...
                internal(thisScopeWrapper)._initialVars.set(elem.variable.name, elem.variable);
                //entryRds.add(elem);
            } else if (Var.isVar(elem)) {
                internal(thisScopeWrapper)._vars.set(elem.name, elem);
                internal(thisScopeWrapper)._initialVars.set(elem.name, elem);
            }
        });
        //if (entryRds.size > 0) {
        //    internal(this).reachIns.set(internal(this)._cfg[0], entryRds);
        //}
    }
};
...
ScopeWrapper.prototype.setVars = function () {
    ...
    if (!!internal(this)._cfg) {
        var thisScope = this;
        internal(thisScope).cfg[2].forEach(function (cfgNode) {
    ...
    if (params instanceof Array || params instanceof Set) {/// if the parameters are valid
        ...
        params.forEach(function (elem) {
            if (Var.isVar(elem)) {
                ...
                internal(thisScope)._vars.set(elem.name, elem);
                internal(thisScope)._params.set(elem.name, elem);
                internal(thisScope)._paramNames.push(elem.name);
            } else if (VarDef.isVarDef(elem)) {
                internal(thisScope)._vars.set(elem.variable.name, elem.variable);
                internal(thisScope)._params.set(elem.variable.name, elem.variable);
                internal(thisScope)._paramNames.push(elem.variable.name.variable);
            }
        });
        ...
        //    //definition of params will be created at the entry point
            //var vardefOfParams = vardefFactory.create(param, defFactory.createParamDef(thisCFGWrapper, Def.LITERAL_TYPE, param.getRange()));
            //if (!!vardefOfParams) {
            //    rdsOfEntry.add(vardefOfParams);
            //}
        //});
        //internal(thisCFGWrapper).reachIns.set(internal(thisCFGWrapper).cfg[0], rdsOfEntry);
    }
};
...
Object.defineProperty(ScopeWrapper.prototype, 'pramName', {
    get: function (index) {
        'use strict';
...
ScopeWrapper.prototype.addChild = function (child) {
    'use strict';
    if (ScopeWrapper.isScopeWrapper(child) && internal(this)._children.indexOf(child) === -1) {/// if the parameter is valid, do
