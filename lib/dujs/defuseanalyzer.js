...
function DefUseAnalyzer() {
    ...
    this._testonly_ = {
        ...
        _getNonReachableVarsAndCorrespondingDefs: getNonReachableVarsAndCorrespondingDefs,
        _getNonReachableDefsAndCorrespondingVars: getNonReachableDefsAndCorrespondingVars,
        ...
        var reachDefinitions = worklist(analysisItem.cfg, function (input) { /// input = ReachIn(n)
            ...
            if (currentNode.type === FlowNode.CALL_NODE_TYPE) {
                ...
                if (currentNode.scope !== currentNode.call.scope.parent) {
                    if (currentNode.scope.hasAscendantContainingTheChild(currentNode.call.scope)) {
                        currentNode.call.scope.spliceUnavailableReachDefinitionsFromSet(input);
                        if (!!availRDs) {
                            input = Set.union(input, availRDs);
                        }
                    } else if (currentNode.call.scope.hasAscendantContainingTheChild(currentNode.scope)) {
                        if (!!availRDs) {
                            input = Set.union(input, availRDs);
                        }
                    }
...
/**
 * Get set of VarDefs where the variables are non-reachable in the scope
 * @param {Set} vardefSet
 * @param {Scope} scope
 * @returns {Set}
 * @memberof DefUseAnalyzer.prototype
 * @private
 */
function getNonReachableVarsAndCorrespondingDefs(vardefSet, scope) {
    "use strict";
    var nonReachable = new Set();
    vardefSet.forEach(function (vardef) {
       if (!scope.hasVariable(vardef.variable.name)) {
...
/**
 * Get the set of VarDefs where the definitions are non-reachable in the scope
 * @param {Set} vardefSet
 * @param {Scope} scope
 * @returns {Set}
 * @memberof DefUseAnalyzer.prototype
 * @private
 */
function getNonReachableDefsAndCorrespondingVars(vardefSet, scope) {
    "use strict";
    var nonReachables = new Set();
    var scopeChain = scope.scopeChain;
    vardefSet.forEach(function (vardef) {
        if (scopeChain.indexOf(vardef.definition.fromNode.scope) === -1) {
            nonReachables.add(vardef);
        }
    });
...
function getNonReachableVarDefs(vardefSet, scope) {
    "use strict";
    var nonReachable = Set.union(
        getNonReachableVarsAndCorrespondingDefs(vardefSet, scope),
        getNonReachableDefsAndCorrespondingVars(vardefSet, scope)
    );
    return nonReachable;
}
