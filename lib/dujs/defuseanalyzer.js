        ...
        if (cfgNode.type === FlowNode.EXIT_NODE_TYPE) {
            ...
            walkes(cfgNode.astNode, {/// Find one AST node in a CFG node at a time
                AssignmentExpression: function (node, recurse) {
                    var killedVar = (node.left.type === 'MemberExpression') ? currentScope.getVariable(node.left.object.name) : currentScope.getVariable(node.left.name);
                    if (!!killedVar) {
                ...
                UpdateExpression: function (node) {
                    var killedVar = currentScope.getVariable(node.argument.name);
                    if (!!killedVar) {
                        redefinedVars.add(killedVar);
                    }
                },
                ...
                VariableDeclarator: function (node) {
                    var initializedVar = currentScope.getVariable(node.id.name);
                    if (!!initializedVar) {
...
DefUseAnalyzer.prototype.updateIntraPageWithLocalStorageDefinition = function (globalScopeCtrl, intraPageAnalysisItems) {
    "use strict";
    if (intraPageAnalysisItems instanceof Array && intraPageAnalysisItems.every(AnalyzedCFG.isModel)) {
