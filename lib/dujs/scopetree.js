...
var CFGExt = require('./cfgext'),
    ...
    factoryVarDef = require('./index').factoryVarDef,
    factoryDef = require('./index').factoryDef,
    factoryVar = require('./index').factoryVar,
    factoryScopeWrapper = require('./index').factoryScopeWrapper,
 ...
 * @constructor
 */
function ScopeTree(ast) {
    'use strict';
    internal(this)._scopes = [];
    internal(this)._numOfAnonymousFunctions = 0;
...
function addScope(tree, scope) {
    ...
    if (ScopeWrapper.isScopeWrapper(scope)) {
        ...
        internal(tree)._mapFromDefToScope.set(scope.def.toString(), scope);
        internal(tree)._mapFromScopeNameToScope.set(scope.scope.toString(), scope);
    }
}
...
function setGlobalScope() {
    ...
    globalScopeWrapper.range = new Range(0,1);
