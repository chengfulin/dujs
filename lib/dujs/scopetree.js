...
/**
 * Update Reach Definitions by adding inter-procedural definitions
 */
FunctionScopeTree.prototype.updateInterProceduralRDs = function () {
    'use strict';
...
function recursivelyUpdateRDs(tree, cfgWrapper) {
    ...
    if (CFGWrapper.isCFGWrapper(currentScope)) {
        currentScope.getCFG().forEach(function (cfgNode) {
            walkes(cfgNode.astNode, {
                ...
                CallExpression: function (node) {
                    ...
                    node.arguments.forEach(function (arg, index) {
                        var rds = new Set(),
                            formalArgument = currentScope.getFunctionParams().get(currentScope.getFunctionParamName(index));
