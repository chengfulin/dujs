...
function CFGWrapper(cfg, scope, parent) {
    ...
    internal(this).def = new Def(
        (!!internal(this).parent)? internal(this).parent.getCFG()[0].cfgId : 0,
        Def.FUNCTION_TYPE,
        internal(this).range,
...
/// TODO: test
CFGWrapper.prototype.updateRDs = function (node, reachIns) {
    'use strict';
    var originalRDs = internal(this).reachIns.get(node),
        resultRDs,
        thisCFGWrapper = this;
    if (!!originalRDs) {
        if (!!reachIns && reachIns instanceof Set) {
            var valid = false;
            reachIns.forEach(function (elem) {
                valid = VarDef.isVarDef(elem);
            });
            if (valid) {
                originalRDs = Set.union(originalRDs, reachIns);
            }
        }
        resultRDs = RDs.findReachDefinitions(this);
        resultRDs.inputs.forEach(function (val, key) {
