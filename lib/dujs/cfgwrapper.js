...
CFGWrapper.prototype.setInitGlobalVars = function (initGlobals) {
    'use strict';
    if (initGlobals instanceof Map) {
        var varsAdded = new Map(),
            thisCFGWrapper = this,
            allString = false,
            allVar = false,
            entryRds = internal(this).reachIns.get(internal(this).cfg[0]) || new Set();
        initGlobals.forEach(function (val, key) {
            Var.validateType(key);
            Def.validateType(val);
            varsAdded.set(key, val);
        });
        varsAdded.forEach(function (val, key) {
            internal(thisCFGWrapper).vars.set(key.getName(), key);
            entryRds.add({variable: key, definition: val});
        });
        if (entryRds.size > 0) {
...
CFGWrapper.prototype.setParams = function (params) {
    ...
    if (params instanceof Array || params instanceof Set) {/// if the parameters are valid
        ...
        paramToAdd.forEach(function (param) {
            ...
            /// for parameters, doesn't know their definition yet
            rdsOfEntry.add({variable: param, definition: null});
        });
        if (rdsOfEntry.size > 0 && rdsOfEntry.size === (params.length || params.size)) {
 ...
 */
CFGWrapper.prototype.initRDs = function () {
    'use strict';
...
CFGWrapper.prototype.toString = function () {
    'use strict';
    return internal(this).getScope() + '@' + internal(this).getRange() +
        (!!internal(this).getParent())? ('_' + internal(this).getParent().getScope() + '@' + internal(this).getParent().getRange()) : '';
};
