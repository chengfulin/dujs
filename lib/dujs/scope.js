...
Scope.prototype.setInitVarDefs = function (initials) {
	...
	if (initials instanceof Array || initials instanceof Set) {
		...
		if (varDefs.size > 0) {
			if (!internal(thisScope)._cfg[0].generate) {
				internal(thisScope)._cfg[0].generate = varDefs;
			} else {
				var originSet = internal(thisScope)._cfg[0].generate;
				varDefs.forEach(function (varDef) {
					originSet.add(varDef);
				});
				internal(thisScope)._cfg[0].generate = originSet;
			}
		}
	}
};
