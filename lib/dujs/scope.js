...
function Scope(ast, name, type, parent) {
	...
	internal(this)._innerFunctionVars = new Map();
...
Scope.validate = function (ast, name, type, parent, msg) {
	'use strict';
	if (!ast.type || Scope.isValidName(name) || Scope.isValidType(type) || !Scope.isValidParent(parent)) {
 ...
 * @returns {String} Contain its type and name
 */
Scope.prototype.toString = function () {
	'use strict';
	return internal(this)._type + ' ' + internal(this)._name;
};
...
Scope.prototype.addInnerFunctionVariable = function (functionName) {
	...
	if (!this.hasLocalVariable(functionName)) {
		...
		internal(this)._innerFunctionVars.set(functionName, functionVariable);
...
Object.defineProperties(Scope.prototype, {
	...
	innerFunctionVars: {
		get: function () {
			'use strict';
			var map = new Map();
			internal(this)._innerFunctionVars.forEach(function (val, key) {
				map.set(key, val);
			});
			return map;
		}
